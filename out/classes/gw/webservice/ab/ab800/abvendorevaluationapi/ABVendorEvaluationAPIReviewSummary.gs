package gw.webservice.ab.ab800.abvendorevaluationapi
...
@Export
@gw.xml.ws.annotation.WsiExportable( "http://guidewire.com/ab/ws/gw/webservice/ab/ab800/abvendorevaluationapi/ABVendorEvaluationAPIReviewSummary" )
  ...
  */
  ...
  private function findReviewSummaryByClaimCenterUID(ccUID : String) : ReviewSummary {
    ...
    if (summaries.getCount() == 0) {
      _logger.debug("findReviewSummaryByClaimCenterUID(): no results for '"+ccUID+"', creating a new ReviewSummary.");
      return null;
    }
    if (summaries.getCount() > 1) {
      _logger.warn("findReviewSummaryByClaimCenterUID(): WARNING "+summaries.getCount()+" results found, should be either 0 or 1.");
    }
    var rs = summaries.getAtMostOneRow();
    _logger.debug("findReviewSummaryByClaimCenterUID(): found summary '"+rs.PublicID+"' for ccuid '"+ccUID+"'.");
    return rs;
  }
  ...
  private function findABContactByLinkID(addressBookUID : String) : ABContact {
    ...
    if (contacts.getCount() > 1) {
      _logger.warn("findABContactByLinkID: nonunique abUID("+addressBookUID+") - duplicates: "+contacts.getCount());
      throw new IllegalStateException("findABContactByLinkID(): nonunique abUID("+addressBookUID+") - duplicates: "+contacts.getCount());
    }
    if (contacts.getCount() == 0) {
      _logger.warn("findABContactByLinkID: nonexistant abUID("+addressBookUID+")");
    ...
    _logger.debug("findABContactByLinkID(): found: "+ contact +"("+addressBookUID+")");
    return contact;
  }
}
