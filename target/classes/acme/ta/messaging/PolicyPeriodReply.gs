...
class PolicyPeriodReply implements MessageReply, MessageListener {
  ...
  override function setDestinationID(destinationID: int) {
      ...
      try {
        _pluginCallbackHandler.execute(\-> {
          ...
          if (aMessage != null && aMessage.AckCount == 0) {
            ...
            if (message.getStringProperty(_MESSAGE_INVALID))  {
              aMessage.reportError()
            }
            if (message.getStringProperty(_TEMP_UNAVAILABLE))  {
              aMessage.reportError(typekey.ErrorCategory.TC_NO_CONNECTION)
            }
            if (message.getStringProperty(_MESSAGE_OK)) {
              aMessage.reportAck()
            }
            if (message.getStringProperty(_MESSAGE_DUPLICATE)) {
