package gw.webservice.ab.ab801
 ...
 */
@WsiWebService("http://guidewire.com/ab/ws/gw/webservice/ab/ab801/MessagingToolsAPI")
...
class MessagingToolsAPI {
  ...
  function purgeCompletedMessages(cutoff : Date) {
    if (cutoff == null) {
      throw new IllegalArgumentException(displaykey.Webservice.Messaging.CutoffDateRequired)
    }
  ...
  function getMessageId(senderRefID : String, destID : int) : long {
    if (destID <= 0 && destID != -1) {
      throw new IllegalArgumentException(displaykey.Webservice.Messaging.DestinationIdInvalid(destID))
    }
  ...
  function ackMessage(ack : Acknowledgement) : boolean {
    if (ack == null || ack.MessageID <= 0) {
      throw new IllegalArgumentException(displaykey.Webservice.Messaging.Ack.Required)
    }
