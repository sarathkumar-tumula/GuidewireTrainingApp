...
@Export
@gw.xml.ws.annotation.WsiExportable( "http://guidewire.com/ab/ws/gw/webservice/ab/ab801/abcontactapi/ABContactAPISearchCriteria" )
  ...
  function toSearchCriteria(b : Bundle) : ABContactSearchCriteria {
    ...
    if (this.SpecialistServiceCodes != null) {
      if(this.SpecialistServiceCodes.Count > ABConfigParameters.MaxNumberServicesInSearchQuery.Value) {
        var errorMessage = displaykey.Java.Contact.Search.TooManyServicesForQuery
        _logger.error(errorMessage)
        throw new SOAPSenderException(errorMessage)
      }
